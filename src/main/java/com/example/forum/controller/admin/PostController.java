  ...
  //      condition.setPostType(PostTypeEnum.POST_TYPE_POST.getValue());
    ...
    public JsonResult pushPost(@ModelAttribute Post post,
        ...
        for (BlackWord blackWord : blackWordList) {
            if (post.getPostTitle().contains(blackWord.getContent())) {
                return JsonResult.error("标题包含违规字符");
            }
            if (tags.contains(blackWord.getContent())) {
                return JsonResult.error("标签包含违规字符");
            }
            if (post.getPostContent().contains(blackWord.getContent())) {
                return JsonResult.error("发表内容包含违规字符");
            }
        }
